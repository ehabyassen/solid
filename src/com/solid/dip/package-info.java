 ...
 * - Abstractions should not depend on details. Details should depend "meet the specification of" on abstractions.
 *
 ...
 * 2- add concretions 'implementations of these interfaces' to the low level modules. So now the low level module depends
 *    on the high level module and must satisfy the contracts own by the high level module.
 *
 * Look at the attached graphs in the package:
 * - The high level module Circle depends on the low level module Drawing, it must be changed if that module changed.
 ...
 *         level module of drawing depends on the abstraction layer own by the high level module class Circle. Circle can
 *         change this interface any time and the implementation details in the low level module must satisfy this
 *         interface.
 *
 ...
 * 4- The dependency inversion is locally in the low level module. The arrow still points from high-level to low-level.
